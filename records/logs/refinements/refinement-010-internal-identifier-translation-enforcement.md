...
status: PRINCIPLE SUBSUMED BY REFINEMENT-013 (2026-05-04). Scan-and-translate MECHANISM preserved (the "before returning, check the draft against the discipline" enforcement step is still active in §7); the RULES it enforces are now refinement-013's audience-based ones, not the surface-based ones in this refinement. Preserved as historical record of the enforcement-layer attempt that preceded the audience-based reframe. Original status: resolved 2026-05-04 — refinement-009 elevated from §5 declaration to §7 enforcement; pre-return scan-and-translate step locked.
