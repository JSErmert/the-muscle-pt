...
type: refinement (final Pre-Alpha cleanup — bypass-message surface + handoff mode-naming)
status: resolved 2026-05-04 — handoff mode-naming drop locked (preserved by refinement-013); bypass-message record-ID translation rule SUPERSEDED by refinement-013 audience-based model (record ID is operator's storage pointer — keep at operator-facing surfaces). Pre-Alpha-7 (post-refinement-012) produced clean artifact body but operator correction reframed bypass-message rule. Pre-Alpha cycle target-completes with refinement-013 + Pre-Alpha-8 fresh test as final pass.
